...
@export_enum('stand', 'walk', 'nesting', 'nest') var starting_pose: String = 'stand'

signal shooed

func _ready() -> void:
	if starting_pose:
		$AnimationPlayer.play(starting_pose)
	#if !manual:
		#$AnimationPlayer.play(waiting_animations.pick_random())

func shoo(broom: Node2D) -> void:
	if state == STATES.waiting:
		state = STATES.fleeing
		shooed.emit()
		print_debug($AnimationPlayer.current_animation)
		if $AnimationPlayer.current_animation == 'nest':
			fall()
		else:
			fly_away(broom)
...
func fly_away(broom: Node2D) -> void:
	...
	$EffectsAnimationPlayer.play("flap")
	var tween = get_tree().create_tween()
